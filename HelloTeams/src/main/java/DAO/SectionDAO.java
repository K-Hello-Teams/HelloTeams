...
public class SectionDAO extends JDBConnPool{
	//섹션 출력
	public List<SectionDTO> getSection(String project_Id) {
		List<SectionDTO> sec_list = new ArrayList();
		String sql = "select * from section where project_Id=?";
		try {
			...
			while(rs.next()) {
				...
				dto.setSection_keep(rs.getString("section_keep"));
				sec_list.add(dto);
			}
	...
	public int addSection(SectionDTO dto) {
		int result = 0;
		String sql = "Insert into section(pro_id,section_id,section_name,host_id,write_id)"
				+ "values(?,?,?,?,?)";
		try {
			psmt = con.prepareStatement(sql);
			psmt.setInt(1,dto.getProject_id());
			psmt.setInt(2, dto.getSection_id());
			psmt.setString(3, dto.getSection_name());
			psmt.setString(4, dto.getWrite_Id());
