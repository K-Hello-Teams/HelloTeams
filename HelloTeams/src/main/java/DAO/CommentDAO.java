...
public class CommentDAO extends JDBConnPool {
	...
	public int insertComment(CommentDTO dto) {
		int result = 0;
		String sql = "INSERT INTO TESTCOMMENT (COM_ID, B_ID, WRITER_ID, COM_CONTENT) VALUES (SEQ_COMMENT_NUM.NEXTVAL, ?, ?, ?)";
		try {
	...
	public List<CommentDTO> getComment(Map<String, Object> map) {
		...
		String sql = "SELECT * FROM (" + "SELECT ROWNUM PNUM, S.* FROM("
				+ "SELECT B.* FROM TESTCOMMENT B WHERE B_ID = ?";
	...
	public int deleteComment(String com_id) {
		int result = 0;
		String sql = "DELETE FROM TESTCOMMENT WHERE COM_ID = ? ";
		try {
			psmt = con.prepareStatement(sql);
	...
	public int countAll(String b_id) {
		int totalCount = 0;
		String sql = "SELECT COUNT(*) FROM TESTCOMMENT WHERE B_ID = ?";
		try {
			psmt = con.prepareStatement(sql);
