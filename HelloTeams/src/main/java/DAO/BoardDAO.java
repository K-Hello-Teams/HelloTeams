...
public class BoardDAO extends JDBConnPool {
	...
	public List<BoardDTO> getListPage(Map<String, Object> map) {
		...
		String sql = "SELECT * FROM (" + "SELECT ROWNUM PNUM, S.* FROM("
				+ "SELECT B.* FROM TESTBOARD B WHERE NOTICEFLAG = ?";
	...
	public int insertWrite(BoardDTO dto) {
		int result = 0;
		String sql = "INSERT INTO TESTBOARD(B_ID, WRITER_ID, TITLE, NOTICEFLAG, CONTENT, BOARDDATE, OFILE, NFILE)"
	...
	// TESTBOARD 테이블 검색 조건까지 고려하여 결과 전체 개수 세기
	...
	public int updatePost(BoardDTO dto) {
		int result = 0;
		try {
			String sql = "UPDATE TESTBOARD SET WRITER_ID=?, TITLE=?, CONTENT=?, OFILE=?, NFILE=? WHERE B_ID=?";
			psmt = con.prepareStatement(sql);
			psmt.setString(1, dto.getWriter_id());
			psmt.setString(2, dto.getTitle());
			psmt.setString(3, dto.getContent());
			psmt.setDate(5, dto.getBoardDate());
...
//			psmt.setInt(8, dto.getDowncount());
	...
	public boolean confirmDelete(String b_id) {
		...
		try {
			String sql = "SELECT COUNT(*) FROM TESTBOARD WHERE B_ID=?";
			psmt = con.prepareStatement(sql);
			psmt.setString(1, b_id);
			rs = psmt.executeQuery();
			rs.next();
	...
	public int deletePost(String b_id) {
		int result = 0;
		try {
			String sql = "DELETE FROM TESTBOARD WHERE B_ID=?";
			psmt = con.prepareStatement(sql);
			psmt.setString(1, b_id);
			result = psmt.executeUpdate();
	...
	public int deleteNoticePost(String noticeFlag) {
		int result = 0;
		try {
			String sql = "DELETE FROM TESTBOARD WHERE NOTICEFLAG=?";
			psmt = con.prepareStatement(sql);
